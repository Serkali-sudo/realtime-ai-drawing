...
public class ColorPickerDialog extends DialogFragment implements ColorPickerView.OnColorChangedListener, TextWatcher {
    ...
    private static final String ARG_SHOW_STROKE_SETTING = "showStrokeSetting";

    private static final String ARG_STROKE_WIDTH = "strokeWidth";
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        showStrokeSetting = getArguments().getBoolean(ARG_SHOW_STROKE_SETTING);
        colorShape = getArguments().getInt(ARG_COLOR_SHAPE);
        strokeWidth = getArguments().getFloat(ARG_STROKE_WIDTH);
    ...
    View createPresetsView() {
        ...
        /*
        Stroke slider and dot view is not coming from the color picking library.
        TODO: Fix stroke preview by using canvas, paint... also keep the scale in mind whatever.
         */

        RangeSlider rangeSlider = contentView.findViewById(R.id.size_slider);
        View dot_view = contentView.findViewById(R.id.dot_view);
        LinearLayout stroke_LL = contentView.findViewById(R.id.stroke_LL);

        if (!showStrokeSetting) {
            stroke_LL.setVisibility(View.GONE);
        }


        rangeSlider.setValues(strokeWidth);
        rangeSlider.setValueFrom(1f);
        rangeSlider.setValueTo(100f);


        ViewGroup.LayoutParams params = dot_view.getLayoutParams();
        params.width = (int) strokeWidth;
        params.height = (int) strokeWidth;
        ...
        rangeSlider.addOnChangeListener(new RangeSlider.OnChangeListener() {
            ...
            public void onValueChange(@NonNull RangeSlider slider, float value, boolean fromUser) {
                ColorPickerDialog.this.onStrokeChanged(value);
                ViewGroup.LayoutParams params = dot_view.getLayoutParams();
    ...
    private void onStrokeChanged(float stroke) {
        if (colorPickerDialogListener != null) {
            Log.w(TAG, "Using deprecated listener which may be remove in future releases");
            colorPickerDialogListener.onStrokeChanged(stroke);
            return;
        }
        Activity activity = getActivity();
        if (activity instanceof ColorPickerDialogListener) {
            ((ColorPickerDialogListener) activity).onStrokeChanged(stroke);
        } else {
            throw new IllegalStateException("The activity must implement ColorPickerDialogListener");
        }
    }
    ...
    public static final class Builder {
        ...
        boolean showStrokeSetting = true;
        ...
        public Builder setShowStroke(boolean showStrokeSetting) {
            this.showStrokeSetting = showStrokeSetting;
            return this;
        }
        ...
        public ColorPickerDialog create() {
            ...
            args.putBoolean(ARG_SHOW_STROKE_SETTING, showStrokeSetting);
